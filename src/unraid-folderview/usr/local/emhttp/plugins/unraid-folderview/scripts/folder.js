...
(async () => {
    ...
    // get folders
    let folders = JSON.parse(await $.get(`/plugins/unraid-folderview/server/read.php?type=${type}`).promise());
    ...
    if (type === 'docker') {
        typeFilter = (e) => {
            return {
                ...
                'Label': e.info.Config.Labels['unraid-folderview'] ?? e.info.Config.Labels['folder.view2']
            }
        };
    } else if (type === 'vm') {
        typeFilter = (e) => {
    ...
    choose = Object.values(JSON.parse(await $.get(`/plugins/unraid-folderview/server/read_info.php?type=${type}`).promise())).map(typeFilter);
