...
const createFolderDocker = (folder, id, position, order, containersInfo, foldersDone) => {
    ...
    // Legacy 'folder.view2' label still honoured -- see docker.js for rationale.
    folder.containers = folder.containers.concat(order.filter(el => {
        const labels = containersInfo[el]?.Labels;
        return (labels?.['unraid-folderview'] ?? labels?.['folder.view2']) === folder.name;
    }));

    // the HTML template for the folder
    const fld = `<div class="folder-showcase-outer-${id} folder-showcase-outer"><span class="outer solid apps stopped folder-docker"><span id="folder-id-${id}" onclick='addDockerFolderContext("${id}")' class="hand docker folder-hand-docker"><img src="${htmlEscape(folder.icon)}" class="img folder-img-docker" onerror="this.src='/plugins/dynamix.docker.manager/images/question.png';"></span><span class="inner folder-inner-docker"><span class="folder-appname-docker">${htmlEscape(folder.name)}</span><br><i class="fa fa-square stopped red-text folder-load-status-docker"></i><span class="state folder-state-docker">${$.i18n('stopped')}</span></span><div class="folder-storage"></div></span><div class="folder-showcase-${id} folder-showcase"></div></div>`;
...
const createFolderVM = (folder, id, position, order, vmInfo, foldersDone) => {
    ...
    // the HTML template for the folder
    const fld = `<div class="folder-showcase-outer-${id} folder-showcase-outer"><span class="outer solid vms stopped folder-vm"><span id="folder-id-${id}" onclick='addVMFolderContext("${id}")' class="hand vm folder-hand-vm"><img src="${htmlEscape(folder.icon)}" class="img" onerror='this.src="/plugins/dynamix.docker.manager/images/question.png"'></span><span class="inner folder-inner-vm">${htmlEscape(folder.name)}<br><i class="fa fa-square stopped red-text folder-load-status-vm"></i><span class="state folder-state-vm">${$.i18n('stopped')}</span></span><div class="folder-storage" style="display:none"></div></span><div class="folder-showcase-${id} folder-showcase"></div></div>`;
...
window.loadlist = (x) => {
    ...
    if($('tbody#vm_view').length > 0) {
        folderReq.vm = [
            // Get the folders
            $.get('/plugins/unraid-folderview/server/read.php?type=vm').promise(),
            // Get the order as unraid sees it
            $.get('/plugins/unraid-folderview/server/read_order.php?type=vm').promise(),
            // Get the info on VMs, needed for autostart and started
            $.get('/plugins/unraid-folderview/server/read_info.php?type=vm').promise(),
            // Get the order that is shown in the webui
            $.get('/plugins/unraid-folderview/server/read_unraid_order.php?type=vm').promise()
        ];
    }
    loadlist_original(x);
};
